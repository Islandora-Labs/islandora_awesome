...
### Viewers
* [Islandora Solr Content Type](https://github.com/BarnardArchives/islandora_solr_content_type) (In Development) - Not necessarily a viewer but instead a way to create a self-contained collection page based on a dynamic Solr query. Has the ability to search and facet within the collection page (without any redirect). Optionally allows the content creator to specify an advanced search query which acts as the base for aforementioned end-user searches.
