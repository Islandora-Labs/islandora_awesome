...
* [Islandora Empty Metadata Values](https://github.com/mjordan/islandora_empty_metadata_values) (Stable) - Utility module that hides metadata elements that are empty or that consist only of puncutation. Works independently of the "Omit Empty Values" option in Islandora Solr's General Configuration settings by using JavaScript to detect if a rendered metadata value is empty (or just contains punctuation).
